...
export async function POST(request: NextRequest) {
  ...
  const validStatuses: BookingStatus[] = ["accepted", "declined", "cancelled", "completed"];
  if (!validStatuses.includes(action as BookingStatus)) {
    return NextResponse.json({ error: `Unknown action: ${action}` }, { status: 400 });
  }

  const { error } = await db
    .from("venue_bookings")
    .update({ status: action as BookingStatus })
    .eq("id", key);
